...
function SheetContent({ children, className, side = "right" }: { children: React.ReactNode; className?: string; side?: "right" | "left" }) {
  ...
  return createPortal(
    <>
      <div 
        className="fixed -inset-4 z-[100] bg-gray-alpha-900 dark:bg-black/70 transition-opacity duration-150 animate-fade-in [will-change:opacity]" 
      ...
      >
        <button
          ...
          onClick={() => onOpenChange(false)}
          className="absolute right-4 top-4 rounded-md p-1 text-ds-text-tertiary opacity-60 transition-all duration-200 hover:bg-gray-alpha-100 hover:opacity-100"
        >
...
function SheetDescription({ className, ...props }: React.HTMLAttributes<HTMLParagraphElement>) {
  return <p className={cn("text-[15px] text-ds-text-secondary", className)} {...props} />;
}
