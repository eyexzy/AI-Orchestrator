...
function SheetContent({ children, className, side = "right" }: { children: React.ReactNode; className?: string; side?: "right" | "left" }) {
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "fixed inset-y-0 z-50 flex w-[380px] max-w-[90vw] flex-col border-white/[0.06] bg-[hsl(0_0%_5%)] shadow-2xl transition-transform duration-300",
      ...
      >
        <button
          ...
          className="absolute right-4 top-4 rounded-md p-1 text-muted-foreground opacity-60 transition-all duration-200 hover:bg-white/[0.05] hover:opacity-100"
        >
