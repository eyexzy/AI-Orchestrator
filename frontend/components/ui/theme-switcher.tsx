...
export type ThemeSwitcherSize = "default" | "small";

interface ThemeSwitcherProps {
  className?: string;
  disabled?: boolean;
  size?: ThemeSwitcherSize;
  onPersist?: (nextTheme: ThemeOption) => void;
  onError?: (error: unknown) => void;
}

type SwitcherMode = "light" | "dark";

const THEME_OPTIONS = ["system", "light", "dark"] as const satisfies readonly ThemeOption[];

const THEME_TOKENS: Record<SwitcherMode, CSSProperties> = {
  light: {
    "--switcher-background-100": "hsla(0, 0%, 100%, 1)",
    "--switcher-gray-1000": "hsla(0, 0%, 9%, 1)",
    "--switcher-gray-700": "hsla(0, 0%, 56%, 1)",
    "--switcher-gray-400": "hsla(0, 0%, 92%, 1)",
    "--switcher-gray-alpha-100": "#0000000d",
    "--switcher-shadow-border": "0 0 0 1px #00000014, 0 0 0 1px hsla(0, 0%, 98%, 1)",
    "--switcher-focus-ring":
      "0 0 0 2px hsla(0, 0%, 100%, 1), 0 0 0 4px hsla(212, 100%, 48%, 1)",
    "--switcher-accents-8": "#111",
    "--switcher-active-shadow":
      "0 0 0 1px hsla(0, 0%, 92%, 1), 0 1px 2px 0 #0000000d",
  } as CSSProperties,
  dark: {
    "--switcher-background-100": "hsla(0, 0%, 4%, 1)",
    "--switcher-gray-1000": "hsla(0, 0%, 93%, 1)",
    "--switcher-gray-700": "hsla(0, 0%, 56%, 1)",
    "--switcher-gray-400": "hsla(0, 0%, 18%, 1)",
    "--switcher-gray-alpha-100": "#ffffff0f",
    "--switcher-shadow-border": "0 0 0 1px #ffffff25, 0 0 0 1px hsla(0, 0%, 0%, 1)",
    "--switcher-focus-ring":
      "0 0 0 2px hsla(0, 0%, 4%, 1), 0 0 0 4px hsla(210, 100%, 66%, 1)",
    "--switcher-accents-8": "#fafafa",
    "--switcher-active-shadow":
      "0 0 0 1px hsla(0, 0%, 18%, 1), 0 1px 2px 0 #ffffff0f",
  } as CSSProperties,
};
...
export function ThemeSwitcher({
  ...
  const currentTheme: ThemeOption = isThemeOption(theme)
    ? theme
    : typeof window !== "undefined" && isThemeOption(localStorage.getItem("theme") ?? undefined)
      ? (localStorage.getItem("theme") as ThemeOption)
      : "system";
  const switcherMode: SwitcherMode = resolvedTheme === "dark" ? "dark" : "light";

  const handleChange = (nextTheme: ThemeOption) => {
    if (disabled || nextTheme === currentTheme) return;
    setTheme(nextTheme);
    onPersist?.(nextTheme);
  };
  ...
  return (
    <div
      ...
      data-size={size}
      style={THEME_TOKENS[switcherMode]}
    >
      {THEME_OPTIONS.map((themeOption) => {
        const id = `theme-switch-${themeOption}-${groupId}`;
        const Icon = THEME_ICONS[themeOption];
        const isChecked = currentTheme === themeOption;
        ...
        return (
          ...
          >
            <span className="theme-switcher__sr-only">{themeOption}</span>
